...
4. 运行时保持卡片开机并靠近 iPhone。App 会自动打开，完成导入、默认六色处理、扫描、
   安全连接和发送；成功以卡片返回的最终 `05 08` ACK 为准。

快捷指令传入的原图只会暂存在 App 的本地目录，读取后立即删除。为保证 CoreBluetooth
传输有足够执行时间，该动作会将 TodooCard 带到前台；锁屏状态或系统不允许打开 App 时，
个人自动化可能无法完成发送。
