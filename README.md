...
- 主页记录最近 12 次成功发送的画面（含分享扩展与快捷指令发出的），轻点原样重发，
  长按可删除单条记录；
...
每次发送成功后，App 会把那张六色画面和当时的 Payload 一起存进最近发送记录，主页“最近发送”
里按时间倒序展示最新的 12 条。轻点缩略图会把原来的 Payload 原样再发一次，不再重新解码和抖动，
卡片上的画面与当初完全一致；长按可以选择“重新发送”或“删除记录”。记录和画面快照一样放在
App Group 容器里，因此分享扩展和快捷指令发出的图片也会出现在这里；免费 Apple Account
重签名后拿不到 App Group 时，主 App 与分享扩展会各自记录在自己的沙盒里。
