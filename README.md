...
- 主页记录最近 12 次成功发送的手动所选图片（含分享扩展所选图片），轻点回到编辑，
  长按可原样重发或删除单条记录；Bing 壁纸、健康摘要与快捷指令自动更新不会挤占列表；
- 与原始仓库一致的颜色 nibble 和面板方向变换；按广播固件选择 v157+ 的 2 KB
  QuickLZ level-3/RAW Payload，旧固件及历史记录继续兼容 QuickLZ stored；
...
- v157+ 使用严格的 32 块耐久 Flash 检查点与窗口末尾 ATT 写入屏障；检查点超时只重发
  窗口末块查询保留进度，不再盲目回放整窗；最终 `05 08` 等待 5 秒仍未到时也只探测性
  重发最后一块，再等待 5 秒后才判定失败；旧固件保留原有节流发送；
- 精确识别乱序包 `0x01`、Flash 写入 `0x09`、回读校验 `0x0A`，并在 v165+ 失败时读取
  保留的 image-ACK 诊断；仍只有最终 `05 08` 才算刷新成功；
...
- 通过“发送今日健康摘要”快捷指令在后台读取健康数据、排版并发送，同样可以配合定时
  自动化每天更新卡片。
- 拿不到 HealthKit 权限的安装方式（例如免费账号重签名），可改用“发送健康摘要（数据由
  快捷指令提供）”，由「快捷指令」App 取数、TodooCard 只负责排版发送。
...
睡眠评分是 TodooCard 在本机估算的：公开的 HealthKit 接口只提供睡眠时段与阶段样本，
没有开放「健康」App 计算出的 Apple 睡眠评分。这里按睡眠时长（对照 8 小时）、深度与
REM 占比、清醒时长和醒来次数加权得到 0–100 分。只有 iPhone 或第三方 App 记录睡眠时
拿不到阶段，评分才只按时长与连续性计算。
...
手动选择或分享的图片发送成功后，App 会把原图副本（长边缩到 2048 的 JPEG）、当时的构图与
效果、六色画面和 Payload 一起存进最近发送记录，主页“最近发送”里按时间倒序展示最新的 12 条，
每条约 1 MB。Bing 壁纸、健康摘要和快捷指令自动更新等生成内容不会写入这里。
...
一次，不重新解码和抖动，卡片上的画面与那次完全一致。

记录和画面快照一样放在 App Group 容器里，因此分享扩展里手动选择的图片也会出现在这里；
...
封装、QuickLZ level-3 固定向量与解压回环、固件感知的线格式选择、传输命令、健康摘要的
圆环与睡眠评分算法，以及固定方向测试图。固定测试图的 legacy 结果必须为：
...
健康摘要需要 `com.apple.developer.healthkit` 权限，而 SideStore 用免费账号重签名后拿不到
它——实测点“生成今日健康摘要”会报 `Missing com.apple.developer.healthkit entitlement`。
用付费开发者账号签名，或在 Xcode 里直接安装（工程已声明 HealthKit capability），就能正常
读取。其余功能不受影响。

拿不到权限时，改用「发送健康摘要（数据由快捷指令提供）」这个动作：取数交给「快捷指令」
App 自己完成，TodooCard 只负责排版和发送，全程不碰 HealthKit API，因此不需要那项权限。

1. 快捷指令 App 新建一个快捷指令，对每一项数据加一个「查找健康样本」动作，选好类型
   （活动能量、锻炼时间、步数、静息心率、睡眠分析等），范围设为今天，再用「计算统计值」
   求和（心率取平均），把结果存进变量。
2. 最后添加 TodooCard 的「发送健康摘要（数据由快捷指令提供）」，把各个变量填进对应参数。
   填几项就画几项，留空的部分卡片会自动省略。
3. 活动和锻炼的目标值手动填一次即可（站立目标在「健身记录」里固定 12 小时，不用填）。

这条路拿不到睡眠分段——在快捷指令里按深度 / 核心 / REM 聚合几乎做不出来——所以只收
睡眠总时长和清醒时长，评分走“没有分段”那条分支，卡片底部会写明。
